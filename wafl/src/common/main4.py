...
from net import Net
from quantize3 import Quantizer3 as Quantizer
from quantize3 import compute_topk_threshold
...
class NetworkTrafficUtils:
    ...
    def save_traffic_metrics(csv_path: str, pcap_path: str, epoch_number: int, neighbours: int) -> None:
        """
        ...
        if epoch_number < 0 or NetworkTrafficUtils.last_saved == epoch_number:
            return
        next_last_timestamp = datetime.datetime.now()
        try:
            inbound = 0
            outbound = 0
            packet_list = []
            ...
            NetworkTrafficUtils.logger.info(f"📝 Traffic Data saved to CSV for epoch {epoch_number}")
        except Exception as e:
            NetworkTrafficUtils.logger.error(f"💥 Failed to save Traffic Data to CSV: {e}")
...
class ModelLearningUtils:
    ...
    def __init__(
        ...
        model_sharing: "EfficientModelSharingUtils",
        ctrl_tcp: "CTRL_TCP",
        experiment_id,
        ...
        self.logger.info(f"📊 CSV files created: {self.csv_file_path} and {self.csv_traf_path}")
    ...
    def network_traffic_thread(self, five_digit_number_str: str, sub_command: str):
        """
        ...
        try:
            ...
            # SELF epochs involve no neighbour exchange, so no traffic is attributed to neighbours.
            if sub_command == "SELF":
                last_n_nbr = 0
            # Kept so the KILL command's final traffic save call has the right epoch to process.
            NetworkTrafficUtils.last_epoch_string = five_digit_number_str
            NetworkTrafficUtils.save_traffic_metrics(self.csv_traf_path, self.pcap_path, self.epoch_number - 1, last_n_nbr)
        except Exception as exc:
    ...
    def self_learn(self, five_digit_number_str: str = "99999", WAFL_LEARN=False) -> bool:
        ...
        try:
            ...
            torch.save(
                self.net.state_dict(),
                self.model_instance_path + ".pth",
            )
            if self.epoch_number in [50, 249, 1049, 5049]:
                torch.save(
                    self.net.state_dict(),
                    self.model_instance_path + f"_{self.epoch_number}.pth",
                )
            self.epoch_number += 1
...
class EfficientModelSharingUtils:
    """
    A class for handling peer-to-peer WAFL model sharing with
    differential sparsification and quantization for reduced network traffic.

    Key features:
    - Tracks previous model state per peer (prev_net_cache)
    - Sends only top-K% of differential (sparsification)
    - Quantizes values to Q bits (quantization)
    """

    cMDLREQ = "MDLREQ"
    # Use bitmap if more than 3% of parameters are non-zero, otherwise use indices
    BITMAP_THRESHOLD = 0.03

    def __init__(self, index: int, name: str, addr: str, port: int, timeout: float,
                 sparsification_K: float = 0.1, quantization_Q: int = 4,
                 num_devices: int = 10) -> None:
        """
        ...
        Args:
            ...
            sparsification_K: Top-K% of parameters to keep (0.0 to 1.0)
            quantization_Q: Number of bits for quantization (1, 2, 4, or 8)
        ...
        self.logger = logging.getLogger("EfficientModelSharingUtils")

        # Efficiency parameters
        self.K = sparsification_K
        self.Q = quantization_Q
        self.quantizer = Quantizer(quantization_Q)
        self.num_devices = num_devices

        # Track previous model state sent to each peer
        # Key: peer_index (int), Value: model state_dict (on CPU)
        self.prev_net_sent: dict = {}

        # Track previous model state received from each peer
        # Key: peer_index (int), Value: model state_dict (on CPU)
        self.prev_net_received: dict = {}

        # Lock for thread-safe access to prev_net caches
        # Using RLock (reentrant lock) to allow nested locking from same thread
        self.cache_lock = threading.RLock()

        self.logger.info(f"Initialized EfficientModelSharingUtils with K={self.K}, Q={self.Q}")
    ...
    def _initialize_prev_net(self, peer_key: str, current_model: dict, cache_type: str) -> dict:
        """
        Initialize prev_net for a peer with zeros (like WAFL-Efficiency zero_initialization).

        Args:
            peer_key: Identifier for the peer (usually IP address)
            current_model: Current model state_dict to get shapes
            cache_type: 'sent' or 'received'

        Returns:
            Initialized zero state_dict
        """
        zero_model = {}
        for key, value in current_model.items():
            zero_model[key] = torch.zeros_like(value, device='cpu')

        with self.cache_lock:
            if cache_type == 'sent':
                self.prev_net_sent[peer_key] = zero_model
            else:
                self.prev_net_received[peer_key] = zero_model

        return zero_model

    def _create_sparse_diff(self, current_model: dict, prev_model: dict) -> dict:
        """
        Create sparsified and quantized differential for transmission.

        Args:
            current_model: Current model state_dict
            prev_model: Previous model state_dict sent to this peer

        Returns:
            Dict containing sparse differential data for each layer
        """
        # Calculate differential for all layers
        diff = {}
        for key in current_model:
            curr_cpu = current_model[key].detach().cpu()
            prev_cpu = prev_model[key].detach().cpu()
            diff[key] = curr_cpu - prev_cpu

        # Compute global threshold for top-K%
        threshold = compute_topk_threshold(diff, self.K)
        self.logger.debug(f"Top-K threshold: {threshold:.6f} (K={self.K})")

        # Create sparse representation for each layer
        sparse_data = {}
        total_params = 0
        sparse_params = 0

        for key, diff_tensor in diff.items():
            flat_diff = diff_tensor.view(-1)
            total_params += flat_diff.numel()

            # Apply threshold mask (keep values with |value| >= threshold)
            mask = flat_diff.abs() >= threshold
            indices = torch.nonzero(mask).squeeze(-1)

            # Handle case where no parameters pass the threshold
            if indices.numel() == 0:
                sparse_data[key] = {
                    'is_empty': True,
            ...
            # If only one element, ensure indices is 1D
            if indices.dim() == 0:
                indices = indices.unsqueeze(0)

            values = flat_diff[indices]
            sparse_params += indices.numel()

            # Quantize values using sign-magnitude quantization
            q_result = self.quantizer.quantize(values)

            # Decide: use bitmap, indices, or nothing based on sparsity
            sparsity_ratio = indices.numel() / flat_diff.numel()

            layer_data = {
                'is_empty': False,
                'shape': list(diff_tensor.shape),
                'numel': flat_diff.numel(),
                'quantized_data': q_result['packed_data'],
                'scale': q_result['scale'],
                'zero_point': q_result['zero_point'],
                'q_numel': indices.numel(),
                'bits': self.Q,
                'all_zero': q_result.get('all_zero', False)
            }

            if self.K >= 1.0 and sparsity_ratio >= 1.0:
                # All parameters are kept: skip the bitmap/indices, saving N/8 bytes per layer.
                layer_data['no_sparsity'] = True
            elif sparsity_ratio > self.BITMAP_THRESHOLD:
                # Use bitmap for high-density sparse data
                layer_data['use_bitmap'] = True
                layer_data['bitmap'] = Quantizer.encode_bitmap(mask)
            else:
                # Use indices for low-density sparse data
                layer_data['use_bitmap'] = False
                layer_data['indices'] = indices.numpy().astype('int32').tobytes()

            sparse_data[key] = layer_data

        self.logger.info(f"📊 Sparsification: {sparse_params}/{total_params} params ({100*sparse_params/total_params:.2f}%)")
        return sparse_data

    def _reconstruct_from_sparse_diff(self, sparse_data: dict, prev_model: dict) -> dict:
        """
        Reconstruct full model from sparse differential and previous model.

        Args:
            sparse_data: Sparse differential data
            prev_model: Previous model state_dict

        Returns:
            Reconstructed full model state_dict
        """
        reconstructed = {}

        for key, layer_data in sparse_data.items():
            prev_tensor = prev_model[key].detach().cpu()

            if layer_data.get('is_empty', False):
                # No changes for this layer
                reconstructed[key] = prev_tensor.clone()
                continue

            shape = layer_data['shape']
            numel = layer_data['numel']

            # Create zero differential tensor
            diff_flat = torch.zeros(numel)

            # Get indices: handle no_sparsity, bitmap, or explicit indices
            if layer_data.get('no_sparsity', False):
                indices = torch.arange(numel)
            elif layer_data.get('use_bitmap', False):
                mask = Quantizer.decode_bitmap(layer_data['bitmap'], numel)
                indices = torch.nonzero(mask).squeeze(-1)
            else:
                indices = torch.from_numpy(
                    np.frombuffer(layer_data['indices'], dtype=np.int32)
                ).long()

            # Dequantize values using sign-magnitude quantization
            quantizer = Quantizer(layer_data['bits'])
            values = quantizer.dequantize(
                layer_data['quantized_data'],
                layer_data['scale'],
                layer_data['zero_point'],
                layer_data['q_numel'],
                layer_data.get('all_zero', False)
            )

            # Scatter values to differential
            diff_flat[indices] = values.float()

            # Reconstruct: prev + diff
            reconstructed[key] = prev_tensor + diff_flat.view(shape)

        return reconstructed

    def _serialize_sparse_diff(self, sparse_data: dict) -> bytes:
        """
        Serialize sparse differential data for transmission.
        """
        self.logger.debug("🔢 Serializing sparse differential for transfer.")
        try:
            serialized_output = pickle.dumps(sparse_data)
            return serialized_output
        except Exception as exc:
            self.logger.error(f"The following error occurred: {str(exc)[:100]}...")
            return b"ERROR"

    def _deserialize_sparse_diff(self, data: bytes) -> dict:
        """
        Deserialize sparse differential data from received bytes.
        """
        self.logger.debug("🔢 De-serializing sparse differential.")
        try:
            deserialized_output = pickle.loads(data)
    ...
    def _compress_data(self, data: bytes) -> bytes:
        """
        Lossless compression of data for transfer.
        """
        self.logger.debug("📦 Compressing data for transfer.")
        try:
            compressed_output = zlib.compress(data)
            original_size_kb = len(data) / 1e3
            compressed_size_kb = len(compressed_output) / 1e3
            self.logger.debug(f"🗜️ Compressed from {original_size_kb:.2f}KB to {compressed_size_kb:.2f}KB.")
    ...
    def _decompress_data(self, data: bytes) -> bytes:
        """
        De-compression of received data using ZLib.
        """
        self.logger.debug("📦 De-compressing received data.")
        try:
            decompressed_output = zlib.decompress(data)
    ...
    def _fetch_model(self, peer_IP: str, other_options: str = "") -> Tuple[bool, Any]:
        ...
        Requests sparse differential from specified peer device.
        """
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                self.logger.debug(f"📥 Requesting WAFL model from peer: {str(peer_IP)}")
                command = f"{EfficientModelSharingUtils.cMDLREQ}:src={self.addr}{other_options}\r\n"
            ...
            sparse_data = self._deserialize_sparse_diff(data)

            if not sparse_data:
                raise Exception("FETCH ERROR: empty sparse data")

            # Get or initialize prev_net for this peer
            with self.cache_lock:
                if peer_IP not in self.prev_net_received:
                    # Initialize with zeros based on current model
                    if self.vMODEL_INSTANCE is not None:
                        self._initialize_prev_net(peer_IP, self.vMODEL_INSTANCE, 'received')
                    else:
                        self.logger.error("Cannot initialize prev_net: no model instance")
                        return False, b"ERROR"
                prev_model = self.prev_net_received[peer_IP]

            # Reconstruct full model from sparse diff
            full_model = self._reconstruct_from_sparse_diff(sparse_data, prev_model)

            # Update prev_net_received for next exchange
            with self.cache_lock:
                self.prev_net_received[peer_IP] = copy.deepcopy(full_model)

            return True, full_model
        except Exception as exc:
    ...
    def _dispatch_model(self, conn: socket.socket, options: str) -> bool:
        """
        Utility function for sending sparse differential to the peer.
        """
        try:
            self.logger.info("⏳ [DISPATCH] Step 1: Starting dispatch preparation")

            if self.vMODEL_INSTANCE is None:
                raise Exception("No model instance available")

            # Extract source IP from options
            src_ip = None
            ...
            if src_ip is None:
                self.logger.warning("Could not extract source IP, using default key")
                src_ip = "unknown"

            self.logger.info(f"⏳ [DISPATCH] Step 2: Source IP = {src_ip}")

            # Get or initialize prev_net for this peer
            with self.cache_lock:
                if src_ip not in self.prev_net_sent:
                    self._initialize_prev_net(src_ip, self.vMODEL_INSTANCE, 'sent')
                prev_model = self.prev_net_sent[src_ip]

            self.logger.info("⏳ [DISPATCH] Step 3: Creating sparse differential...")
            # Create sparse differential
            sparse_data = self._create_sparse_diff(self.vMODEL_INSTANCE, prev_model)

            # Reconstruct the model as the receiver will see it after dequantizing, so the
            # sender's cached prev_net stays exactly in sync with the receiver's state.
            reconstructed_model = self._reconstruct_from_sparse_diff(sparse_data, prev_model)

            self.logger.info("⏳ [DISPATCH] Step 4: Updating prev_net_sent cache with reconstructed model")
            with self.cache_lock:
                self.prev_net_sent[src_ip] = copy.deepcopy(reconstructed_model)

            self.logger.info("⏳ [DISPATCH] Step 5: Serializing sparse differential")
            # Serialize and compress
            data = self._serialize_sparse_diff(sparse_data)

            self.logger.info("⏳ [DISPATCH] Step 6: Compressing data")
            data = self._compress_data(data)

            if data == b"ERROR":
                raise Exception("DISPATCH ERROR: serialization failed")

            self.logger.info(f"⏳ [DISPATCH] Step 7: Sending {len(data)} bytes to peer")
            conn.sendall(data)
            self.logger.info("✅ [DISPATCH] Successfully sent sparse differential to the peer.")
            return True
        except Exception as exc:
            self.logger.error(f"The following error occurred in _dispatch_model: {str(exc)[:100]}...")
            return False

    def _handle_dispatch_request(self, conn: socket.socket, options: str) -> None:
        """
        Handle a dispatch request in a separate thread to avoid blocking the listener.
        """
        try:
            DISPATCHED = self._dispatch_model(conn, options)
            if not DISPATCHED:
                self.logger.error("Dispatch failed for request")
        except Exception as exc:
            self.logger.error(f"Error in dispatch thread: {str(exc)[:100]}...")
        finally:
            try:
                conn.close()
            except Exception:
    ...
    def _socket_listener_thread(self) -> None:
        """
        Implementation of the Peer-to-Peer Sharing Listener Thread.
        """
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            # Allow reuse of address even if previous socket is in TIME_WAIT state
            ...
            while self.fLISTENER_ACTIVE:
                try:
                    ...
                    if command != EfficientModelSharingUtils.cMDLREQ:
                        raise Exception("COMMAND MISMATCH")
                    self.logger.info("📡 Dispatching Sparse Differential to the WAFL Peer.")
                    # Spawn a new thread for dispatch to avoid blocking the listener
                    dispatch_thread = threading.Thread(
                        target=self._handle_dispatch_request,
                        args=(conn, options),
                        daemon=True
                    )
    ...
    def update_model_instance(self, LE_model: Any, metadata: str = "") -> None:
        """
        Updates the WAFL model instance.
        Note: Cache is not used for sparse differential approach.
        """
        # Convert to CPU for consistent storage
        cpu_model = {}
        for key, value in LE_model.items():
            cpu_model[key] = value.detach().cpu()
        self.vMODEL_INSTANCE = cpu_model
        self.vMODEL_INSTANCE_CACHE = None  # Not used in efficient mode
        self.vMODEL_METADATA = metadata

    def request_model_from_peer(self, peer_IP: str, other_options: str = "") -> Any:
        """
        Retrieve model parameters from a WAFL peer device.
...
class CTRL_TCP:
    ...
    def __init__(self, config_path: str):
        ...
        self.model_sharing: Optional[EfficientModelSharingUtils] = None
    ...
    def setup_local_wafl_node(self, agent_index: int, device_name: str) -> None:
        ...
        # Get efficiency parameters from wafl_phase_params (with defaults)
        sparsification_K = self.wafl_phase_params.get("sparsification_K", 0.1)
        quantization_Q = self.wafl_phase_params.get("quantization_Q", 4)
        num_devices = len(self.device_names) if self.device_names else 10

        self.model_sharing = EfficientModelSharingUtils(
            agent_index, device_name, device_ip, self.p2p_port, self.timeout,
            sparsification_K=sparsification_K,

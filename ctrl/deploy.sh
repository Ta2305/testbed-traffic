...
# SSH connection settings: fail fast on unreachable hosts, keep long-running
# transfers alive, and skip host key prompts across many unattended devices
SSH_OPTS="-o ConnectTimeout=10 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 -o StrictHostKeyChecking=no"

# SSH multiplexing: reuse one TCP connection per host for all the ssh/scp
# calls below instead of renegotiating a new connection for each one
...
# Clear the list of failed deployments from any previous run
...
deploy_to_device() {
    ...
    # Close the SSH master connection once this function returns, whether
    # it succeeds or fails, so multiplexed sockets don't leak in $CTL_DIR
    ...
    # Establish the master connection that the ssh/scp calls below will share
    ...
    # Transfer each file group only if it's non-empty; ERROR_CHECK is bumped
    # before a transfer and released after it succeeds, so it settles back to
    # 0 only if every attempted transfer succeeded

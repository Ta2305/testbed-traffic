...
class WaflAgent:
    ...
    def _deploy_configurations(self, experiment_parameters: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            with paramiko.SSHClient() as ssh:
                ...
                ssh.connect(self.ip, port=ssh_port, username=username, allow_agent=True, timeout=10)
    ...
    def start_remote_process(self, experiment_id: str, ssh_password: str, script_name: str = "src/main4.py") -> bool:
        """
        ...
        try:
            ...
            python_script = os.path.join(target_path, script_name)
    ...
    def _send_command(self, command: str) -> Tuple[bool, str]:
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                # Allow reuse of address even if in TIME_WAIT state
...
class ControlServer:
    ...
    def _load_shell_env_vars(self, config_path: str):
        ...
        try:
            # Preserve CLI-provided EXPERIMENT_NAME before loading shell env vars
            cli_experiment_name = os.environ.get("EXPERIMENT_NAME") if os.environ.get("_CLI_EXPERIMENT_NAME_SET") else None
            ...
            # Restore CLI-provided EXPERIMENT_NAME if it was set
            if cli_experiment_name is not None:
                os.environ["EXPERIMENT_NAME"] = cli_experiment_name
                self.logger.debug(f"🔧 Restored CLI EXPERIMENT_NAME: {cli_experiment_name}")
    ...
    def run_experiment(
        ...
        try:
            ...
            # Get SUDO password: from argument, env var, or prompt
            if sudo_password:
                ssh_password = sudo_password
                self.logger.info("🔑 Using sudo password from command line argument")
            elif os.environ.get("SUDO_PASSWORD"):
                ssh_password = os.environ.get("SUDO_PASSWORD")
                self.logger.info("🔑 Using sudo password from SUDO_PASSWORD environment variable")
            else:
            ...
            for agent in self.agents:
                if not agent.start_remote_process(self.experiment_id, ssh_password, script_name=script_name):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="WAFL Experiment Controller - Run federated learning experiments",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use default parameters from files
  python ctrl/main.py

  # Override specific parameters
  python ctrl/main.py --experiment_name my_exp --self_epoch 128 --wafl_epoch 2048

  # Override efficiency parameters
  python ctrl/main.py --K 0.1 --Q 4

  # Full custom run
  python ctrl/main.py --experiment_name test_K01_Q4 --self_epoch 100 --wafl_epoch 1000 --K 0.1 --Q 4
        """
    )
    parser.add_argument(
        "--experiment_name",
        type=str,
        default=None,
        help="Experiment name (overrides EXPERIMENT_NAME in execution_config)"
    )
    parser.add_argument(
        "--self_epoch",
        type=int,
        default=None,
        help="Number of self-learning epochs (overrides epochs.self in parameters.json)"
    )
    parser.add_argument(
        "--wafl_epoch",
        type=int,
        default=None,
        help="Number of WAFL epochs (overrides epochs.wafl in parameters.json)"
    )
    parser.add_argument(
        "--K",
        type=float,
        default=None,
        help="Sparsification rate (0.0-1.0, overrides wafl_phase.sparsification_K in parameters.json)"
    )
    parser.add_argument(
        "--Q",
        type=int,
        default=None,
        help=(
            "Quantization bits (0, 1, 2, 4, 8, 16, 32... 0=no quantization, "
            "overrides wafl_phase.quantization_Q in parameters.json)"
        )
    )
    parser.add_argument(
        "--wafl_script",
        type=str,
        default="src/main4.py",
        help=(
            "Path to the python script to run on remote nodes (relative to project root, "
            "e.g. src/main.py for the uncompressed baseline or src/main4.py for Top-K sparsification + quantization)"
        )
    )
    parser.add_argument(
        "--sudo_password",
        type=str,
        default=None,
        help="Sudo password for remote commands (can also set SUDO_PASSWORD env var). If not provided, will prompt."
    )
    ...
    try:
        ...
        if args.self_epoch is not None:
            print(f"🔧 Overriding self_epoch: {experiment_parameters['epochs']['self']} -> {args.self_epoch}")
            experiment_parameters["epochs"]["self"] = args.self_epoch

        if args.wafl_epoch is not None:
            print(f"🔧 Overriding wafl_epoch: {experiment_parameters['epochs']['wafl']} -> {args.wafl_epoch}")
            experiment_parameters["epochs"]["wafl"] = args.wafl_epoch

        if args.K is not None:
            old_K = experiment_parameters["wafl_phase"].get("sparsification_K", "not set")
            print(f"🔧 Overriding sparsification_K: {old_K} -> {args.K}")
            experiment_parameters["wafl_phase"]["sparsification_K"] = args.K

        if args.Q is not None:
            old_Q = experiment_parameters["wafl_phase"].get("quantization_Q", "not set")
            print(f"🔧 Overriding quantization_Q: {old_Q} -> {args.Q}")
            experiment_parameters["wafl_phase"]["quantization_Q"] = args.Q

        # Override EXPERIMENT_NAME in environment if provided
        if args.experiment_name is not None:
            print(f"🔧 Overriding EXPERIMENT_NAME: {os.environ.get('EXPERIMENT_NAME', 'not set')} -> {args.experiment_name}")
            os.environ["EXPERIMENT_NAME"] = args.experiment_name
            os.environ["_CLI_EXPERIMENT_NAME_SET"] = "1"  # Flag to preserve across config loading

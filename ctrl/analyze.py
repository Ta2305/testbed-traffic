...
def get_epoch_ranges(experiment_path: str) -> dict:
    """
    Returns the SELF/WAFL epoch boundary used to annotate plots.

    These are fixed values rather than being read from `parameters.json`,
    because that file can be overridden by command-line arguments at
    experiment time and no longer reflect what actually ran.
    """
    return {"self": 128, "wafl": 2048}
